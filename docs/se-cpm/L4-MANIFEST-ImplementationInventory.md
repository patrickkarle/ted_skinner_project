...
#### IM-3015: LLMClient::generate_stream()
**Location:** `src-tauri/src/llm/mod.rs`
**Type:** Async method
**L3 Reference:** L3-CDD-03 Section 4.4
**Dependencies:** IM-3001 (LLMRequest), IM-3013 (detect_provider), IM-3401 (LLMProvider::generate_stream)

##### Variable-Level Inventory

#### IM-3015-V1: provider_name Variable
**Type:** &str
**Initialization:** Result from detect_provider()
**Scope:** Provider routing
**Purpose:** Determine which LLM provider to use

#### IM-3015-V2: provider Variable
**Type:** Arc<dyn LLMProvider>
**Initialization:** Clone from self.providers HashMap
**Scope:** Provider instance for streaming
**Purpose:** Delegate streaming to provider-specific implementation

##### Branch-Level Inventory

#### IM-3015-B1: Provider Found Check
**Condition:** Provider exists in self.providers HashMap
**True Path:** Clone provider (IM-3015-V2)
**False Path:** Return IM-3015-E1 (UnsupportedModel error)

##### Error-Level Inventory

#### IM-3015-E1: Unsupported Model Error
**Trigger:** Provider not found in HashMap
**Error Type:** LLMError::UnsupportedModel
**Message:** "Unsupported model: {model_name}"

#### IM-3020: RateLimiter Struct
**Location:** `src-tauri/src/llm/rate_limiter.rs`
**Type:** Rust struct
**L3 Reference:** L3-CDD-03 Section 7.2
**Dependencies:** None (std lib only)

#### IM-3020-F1: tokens Field
**Type:** f64
**Purpose:** Current token count (available requests)
**Mutability:** Mutable (decremented on acquire, refilled over time)

#### IM-3020-F2: capacity Field
**Type:** f64
**Purpose:** Maximum tokens (requests per minute limit)
**Mutability:** Immutable after construction

#### IM-3020-F3: refill_rate Field
**Type:** f64
**Purpose:** Tokens added per second (capacity / 60.0)
**Mutability:** Immutable after construction

#### IM-3020-F4: last_refill Field
**Type:** Instant
**Purpose:** Timestamp of last token refill operation
**Mutability:** Mutable (updated on each refill)

#### IM-3021: RateLimiter::new()
**Location:** `src-tauri/src/llm/rate_limiter.rs`
**Type:** Constructor
**L3 Reference:** L3-CDD-03 Section 7.2
**Dependencies:** IM-3020

##### Variable-Level Inventory

#### IM-3021-P1: requests_per_minute Parameter
**Type:** f64
**Validation:** Should be > 0.0 (not enforced, caller's responsibility)
**Purpose:** Rate limit capacity (e.g., 50.0 for Claude, 100.0 for DeepSeek)

#### IM-3022: RateLimiter::try_acquire()
**Location:** `src-tauri/src/llm/rate_limiter.rs`
**Type:** Method
**L3 Reference:** L3-CDD-03 Section 7.2
**Dependencies:** IM-3023 (refill)

##### Variable-Level Inventory

#### IM-3022-V1: wait_seconds Variable
**Type:** f64
**Initialization:** (1.0 - self.tokens) / self.refill_rate
**Scope:** Wait time calculation when rate limited
**Purpose:** Calculate duration to wait for next token

##### Branch-Level Inventory

#### IM-3022-B1: Token Available Check
**Condition:** self.tokens >= 1.0
**True Path:** Decrement tokens, return Ok(())
**False Path:** Calculate wait time, return Err(Duration)

##### Error-Level Inventory

#### IM-3022-E1: Rate Limit Exceeded
**Trigger:** self.tokens < 1.0
**Error Type:** Err(Duration) (wait time until next token)
**Message:** None (caller interprets Duration)

#### IM-3023: RateLimiter::refill()
**Location:** `src-tauri/src/llm/rate_limiter.rs`
**Type:** Private method
**L3 Reference:** L3-CDD-03 Section 7.2
**Dependencies:** IM-3020-F3 (refill_rate), IM-3020-F4 (last_refill)

##### Variable-Level Inventory

#### IM-3023-V1: now Variable
**Type:** Instant
**Initialization:** Instant::now()
**Scope:** Current timestamp
**Purpose:** Calculate elapsed time since last refill

#### IM-3023-V2: elapsed Variable
**Type:** f64
**Initialization:** now.duration_since(self.last_refill).as_secs_f64()
**Scope:** Time since last refill
**Purpose:** Calculate how many tokens to add

#### IM-3024: RateLimiter::available_tokens()
**Location:** `src-tauri/src/llm/rate_limiter.rs`
**Type:** Method
**L3 Reference:** L3-CDD-03 Section 7.2
**Dependencies:** IM-3020-F1 (tokens field)

#### IM-3030: CircuitBreaker Struct
**Location:** `src-tauri/src/llm/circuit_breaker.rs`
**Type:** Rust struct
**L3 Reference:** L3-CDD-03 Section 7.3
**Dependencies:** IM-3031 (CircuitState enum)

#### IM-3030-F1: state Field
**Type:** CircuitState
**Purpose:** Current circuit state (Closed, Open, HalfOpen)
**Mutability:** Mutable (transitions between states)

#### IM-3030-F2: failure_count Field
**Type:** u32
**Purpose:** Consecutive failures counter
**Mutability:** Mutable (incremented on failure, reset on success)

#### IM-3030-F3: failure_threshold Field
**Type:** u32
**Purpose:** Failures before opening circuit (5 from L1-SAD)
**Mutability:** Immutable after construction

#### IM-3030-F4: success_count Field
**Type:** u32
**Purpose:** Consecutive successes counter (in HalfOpen state)
**Mutability:** Mutable (incremented on success, reset on failure)

#### IM-3030-F5: success_threshold Field
**Type:** u32
**Purpose:** Successes before closing circuit (2 from L1-SAD)
**Mutability:** Immutable after construction

#### IM-3030-F6: open_until Field
**Type:** Option<Instant>
**Purpose:** Timestamp when circuit can transition to HalfOpen
**Mutability:** Mutable (set when opening circuit)

#### IM-3030-F7: timeout_duration Field
**Type:** Duration
**Purpose:** How long to stay Open before trying HalfOpen (60s from L1-SAD)
**Mutability:** Immutable after construction
...
#### IM-3031-V1: Closed Variant
**Purpose:** Normal operation, all requests allowed

#### IM-3031-V2: Open Variant
**Purpose:** Blocking all requests due to failures

#### IM-3031-V3: HalfOpen Variant
**Purpose:** Allowing test request to check provider recovery

#### IM-3032: CircuitBreakerError Enum
**Location:** `src-tauri/src/llm/circuit_breaker.rs`
**Type:** Rust error enum
**L3 Reference:** L3-CDD-03 Section 7.3

#### IM-3032-V1: Open Variant
**Purpose:** Circuit is open, request rejected immediately

#### IM-3032-V2: RequestFailed Variant
**Type:** Box<dyn std::error::Error>
**Purpose:** Request failed, counted toward threshold

#### IM-3033: CircuitBreaker::new()
**Location:** `src-tauri/src/llm/circuit_breaker.rs`
**Type:** Constructor
**L3 Reference:** L3-CDD-03 Section 7.3
**Dependencies:** IM-3030, IM-3031

#### IM-3034: CircuitBreaker::call()
**Location:** `src-tauri/src/llm/circuit_breaker.rs`
**Type:** Method (generic over function F and return type T)
**L3 Reference:** L3-CDD-03 Section 7.3
**Dependencies:** IM-3035 (on_success), IM-3036 (on_failure)

##### Branch-Level Inventory

#### IM-3034-B1: Circuit State Check
**Condition:** self.state matches CircuitState::Open
**True Path:** Check if timeout expired (IM-3034-B2)
**False Path:** Proceed with function execution (IM-3034-B4)

#### IM-3034-B2: Timeout Expiration Check
**Condition:** Instant::now() >= self.open_until.unwrap()
**True Path:** Transition to HalfOpen, proceed with function
**False Path:** Return IM-3034-E1 (circuit still open)

#### IM-3034-B3: Function Execution Result
**Condition:** f() returns Ok(result)
**True Path:** Call on_success(), return Ok(result)
**False Path:** Call on_failure(), return Err(CircuitBreakerError::RequestFailed)

##### Error-Level Inventory

#### IM-3034-E1: Circuit Open Error
**Trigger:** Circuit in Open state, timeout not expired
**Error Type:** CircuitBreakerError::Open
**Message:** None (caller interprets error variant)

#### IM-3035: CircuitBreaker::on_success()
**Location:** `src-tauri/src/llm/circuit_breaker.rs`
**Type:** Private method
**L3 Reference:** L3-CDD-03 Section 7.3
**Dependencies:** IM-3030 (fields)

##### Branch-Level Inventory

#### IM-3035-B1: State Check for HalfOpen
**Condition:** self.state == CircuitState::HalfOpen
**True Path:** Increment success_count, check threshold (IM-3035-B2)
**False Path:** No action (remain Closed)

#### IM-3035-B2: Success Threshold Reached
**Condition:** self.success_count >= self.success_threshold
**True Path:** Transition to Closed, reset success_count
**False Path:** Remain in HalfOpen

#### IM-3036: CircuitBreaker::on_failure()
**Location:** `src-tauri/src/llm/circuit_breaker.rs`
**Type:** Private method
**L3 Reference:** L3-CDD-03 Section 7.3
**Dependencies:** IM-3030 (fields)
...
#### IM-3036-B1: Failure Threshold Reached
**Condition:** self.failure_count >= self.failure_threshold
**True Path:** Transition to Open, set open_until timestamp
**False Path:** Remain in current state, increment failure_count

#### IM-3037: CircuitBreaker::state()
**Location:** `src-tauri/src/llm/circuit_breaker.rs`
**Type:** Method
**L3 Reference:** L3-CDD-03 Section 7.3
**Dependencies:** IM-3030-F1 (state field)
...
#### IM-3400-M1: name() Method
**Type:** Synchronous method
**Return:** &str
**Purpose:** Return provider name ("anthropic", "google", "deepseek")

#### IM-3400-M2: generate() Method
**Type:** Async method
**L3 Reference:** L3-CDD-03 Section 5.1
**Dependencies:** IM-3001 (LLMRequest), IM-3002 (LLMResponse)
**Purpose:** Generate text response from LLM (blocking, returns full response)

#### IM-3401: LLMProvider::generate_stream() Method
**Type:** Async method (trait method)
**L3 Reference:** L3-CDD-03 Section 4.4 (Provider Trait Extension)
**Dependencies:** IM-3001 (LLMRequest)
**Purpose:** Stream tokens as they arrive for UI progress updates

##### Return Type Details

#### IM-3401-R1: Stream Return Type
**Type:** Pin<Box<dyn Stream<Item = Result<String>> + Send>>
**Purpose:** Asynchronous stream of text deltas
**Protocol:** Server-Sent Events (SSE) for Anthropic/DeepSeek, JSON stream for Gemini
